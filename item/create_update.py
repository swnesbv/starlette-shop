...
async def parent_create(request, model, obj, img):
    ...
    async with async_session() as session:
        ...
        if request.method == "POST":
            ...
            on = form.getlist("categories")
            categories = []
            for (c, d) in zip(on_off, on):
                if c == "1" :
                    categories.append(d)
            # ..
...
async def child_create(request, context, form, model, new, item, re_item):
    # ..
...
async def child_img_create(request, form, belongs, model, new, item, re_item, img):
    # ..
    basewidth = 800
    template = f"/{item}/create.html"
    ...
    async with async_session() as session:
        ...
        if request.method == "POST":
            ...
            # ..
            on_off = form.getlist("on_off")
            on_categories = form.getlist("categories")
            on_cts = form.getlist("cts")
            categories = []
            cts = []
            for (c, d) in zip(on_off, on_categories):
                if c == "1" :
                    categories.append(d)
            for (c, d) in zip(on_off, on_cts):
                if c == "1" :
                    cts.append(d)
            # ..
            if file.filename == "":
...
async def child_img_update(request, form, context, model, id, item, img, amount, price):
    # ..
    basewidth = 800
    template = f"/{item}/update.html"
